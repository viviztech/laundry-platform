...
class Command(BaseCommand):
    ...
    def seed_pricing_zones(self):
        ...
        zones_data = [
            {'name': 'Central Bangalore', 'code': 'BLR-CENTRAL', 'multiplier': Decimal('1.2')},
            {'name': 'North Bangalore', 'code': 'BLR-NORTH', 'multiplier': Decimal('1.0')},
            {'name': 'South Bangalore', 'code': 'BLR-SOUTH', 'multiplier': Decimal('1.1')},
            {'name': 'East Bangalore', 'code': 'BLR-EAST', 'multiplier': Decimal('0.9')},
            {'name': 'West Bangalore', 'code': 'BLR-WEST', 'multiplier': Decimal('1.0')},
        ]
    ...
    def seed_orders(self, users, services, partners):
        ...
        for i in range(30):  # Create 30 orders
            ...
            address = UserAddress.objects.filter(user=customer, is_default=True).first()
            if not address:
                continue
